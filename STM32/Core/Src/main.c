	  ...
	  if (input_mode) // Если true то используем потенциометры для управления
	      {
	        ...
	        HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_RESET);  // Выключить LED PB3
	        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_2, adc_values[0]/4.885);
	        __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, adc_values[1]/4.885);
	      }
	      else // Если false то используем заготовленный сигнал sin
	      {
	        ...
	        HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_SET);    // Включить LED PB3
	      }
